...
foreach($hospital['departments'] as $department) {
	...
	foreach($department['location'] as $location) {
		...
		$locations[$key] = '<a href="javascript:;" onClick="gpt_blog(\''.$location->city.'\', \''.urlencode($hospital['name']).'\')" data-city="'.$location->city.'">'.$location->city.', '.$location->country.' <i class="fa fa-external-link" aria-hidden="true"></i></a>';		
	}
