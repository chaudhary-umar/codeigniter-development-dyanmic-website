...
foreach($contacts as $contact){
    ...
    foreach($addresses as $address) {
        ...
        $locations[$key] = '<a href="javascript:;" onClick="gpt_blog(\''.$location->city.'\', \''.urlencode($serviceProvider->getCompanyName()).'\')" data-city="'.$location->city.'">'.$location->city.', '.$location->country.' <i class="fa fa-external-link" aria-hidden="true"></i></a>';				
    }
}
ksort($services);
ksort($locations);
